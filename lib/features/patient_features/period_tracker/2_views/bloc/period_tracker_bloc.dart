...
class PeriodTrackerBloc extends Bloc<PeriodTrackerEvent, PeriodTrackerState> {
  final PeriodTrackerController controller;
  PeriodTrackerBloc(this.controller, {required Object periodTrackerController})
  ...
  FutureOr<void> fetchPeriodData(
    ...
    try {
      await controller.fetchPeriods();
      emit(PeriodTrackerLoaded(controller.periods));
    } catch (e) {
  ...
  FutureOr<void> logPeriod(
    ...
    try {
      await controller.logPeriod(event.startDate, event.endDate);
      add(FetchPeriodData());
    } catch (e) {
      emit(PeriodTrackerError(e.toString()));
    }
  ...
  FutureOr<void> deletePeriod(
    ...
    try {
      await controller.deletePeriod(event.id);
      add(FetchPeriodData());
    } catch (e) {
      emit(PeriodTrackerError(e.toString()));
    }
